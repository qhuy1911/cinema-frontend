...
function Movie() {
  ...
  return(
    <>
    {movie.id==id?(<div className="movie_flex">
          <div className="movie_imgage"><img src={require(`../../../assets/images/${movie.image}`)} alt="Not found" /></div>
          <div className="movie_info">
            <h2 className="movie_title">{movie.name}</h2>
            <div className='movie_director'>
              <span>Đạo diễn: <span style={{color:"#e63757"}}>{movie.director}</span></span>
            </div>
            <p className="movie_description">{movie.description}</p>
            <div className="info_time">
              <div className='info-startDate'>
                <p>Khởi chiếu </p>
                <p><FontAwesomeIcon icon={faCalendar} /> {movie.startDate}</p>
              </div>
              <div className='info-duration'>
                <span>Thời lượng</span>
                <p><FontAwesomeIcon icon={faClock} /> {movie.duration} phút</p>
              </div>
            </div>
          </div>
